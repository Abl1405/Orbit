...
class Agent:
	def __init__(self, states, actions, learning_rate=0.9, reward_decay=0.9, e_greedy=0.9):
		self.actions = actions
		self.states = states
		self.lr = learning_rate
		self.gamma = reward_decay
		self.epsilon = e_greedy
		self.q_table = pd.DataFrame(columns=self.actions, index=self.states, dtype=np.float64)
		self.q_table_final = pd.DataFrame(columns=self.actions, index=self.states, dtype=np.float64)
